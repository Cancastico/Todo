  ...
  return (
    <main className="w-full flex-wrap flex flex-row justify-center lg:justify-normal ">
      {tasks.map((task, index) => {
        return (
          <Card key={index} className="relative w-[80%] h-[18rem] outline-primary flex flex-col justify-between shadow-primary shadow-xl  lg:ml-[4%] my-[2%] lg:w-[20%] ">
            {!task.completed &&
              <Trash2 onClick={() => { exclude(task.id) }} className="absolute transition-colors duration-300 right-[1.6rem] top-[1.4rem] text-zinc-300 hover:text-red-500 " />
            }
